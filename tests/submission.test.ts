...
describe("PublicStuff submission response", () => {
  ...
  it("records a structured PublicStuff rejection returned with HTTP 200", async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValue(new Response(JSON.stringify({
      response: { status: { type: "error", code: 422, message: "Error" } },
    }), { status: 200 }));
    ...
    })).rejects.toMatchObject({ kind: "validation" } satisfies Partial<SubmissionError>);
